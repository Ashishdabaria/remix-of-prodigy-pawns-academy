...
import { PAWN_VILLAGE_LEVELS, type PathLevel } from "@/data/realm1/path-levels";
...
// 12 nodes laid out along a gentle winding path (percent of width).
...
function RealmPathPage() {
  ...
  const levels = PAWN_VILLAGE_LEVELS;

  // Stack levels vertically with consistent gap; bottom = first level.
  const ROW = 160; // px between nodes
  const PAD_TOP = 120;
  const PAD_BOTTOM = 120;
  const height = PAD_TOP + PAD_BOTTOM + (levels.length - 1) * ROW;

  // Build SVG path connecting nodes (bottom-up).
    ...
    <div className="relative min-h-screen overflow-hidden">
      ...
      <header className="sticky top-0 z-20 flex items-center justify-between gap-3 border-b-2 border-ink/15 bg-parchment/85 px-4 py-3 backdrop-blur">
        ...
        <div className="rounded-full bg-shard-sun px-3 py-1 text-xs font-black text-ink ring-2 ring-ink/20">
          {levels.filter((l) => l.status === "done").length}/{levels.length}
        </div>
      </header>
      ...
      >
        ...
        {/* Mariposa hovers near the current level */}
        {(() => {
          const i = levels.findIndex((l) => l.status === "current");
          if (i < 0) return null;
          const p = points[i];
          return (
            <div
              className="pointer-events-none absolute -translate-x-1/2"
              style={{ left: `${p.x}%`, top: p.y - 64 }}
            >
              <Mariposa size={48} />
            </div>
          );
        ...
        {levels.map((lvl, i) => {
          ...
          return (
            ...
            >
              <LevelNode level={lvl} realmId={realm.id} />
            </div>
          );
        })}
      </div>
...
function LevelNode({ level, realmId }: { level: PathLevel; realmId: string }) {
  const { status, name, blurb, icon, stars = 0 } = level;
  ...
  const Inner = (
    <div className="flex flex-col items-center gap-1.5">
      ...
      <div className="min-w-[10rem] max-w-[12rem] rounded-xl border-2 border-ink/20 bg-parchment/95 px-3 py-1.5 text-center shadow-md">
        <div className="font-display text-xs font-black leading-tight text-ink">{name}</div>
        <div className="mt-0.5 text-[10px] font-bold text-ink/60">{blurb}</div>
        ...
        {status === "done" && (
          <div className="mt-1 flex items-center justify-center gap-0.5" aria-label={`${stars} of 3 stars`}>
            {[1, 2, 3].map((s) => (
              <span
                key={s}
                className={s <= stars ? "text-shard-sun" : "text-ink/20"}
                aria-hidden
              >
                ★
              </span>
  ...
  if (tappable) {
    return (
      <Link
        ...
        params={{ realmId }}
        aria-label={`Level ${level.id}: ${name}. Start.`}
  ...
  return (
    <div
      aria-disabled
      aria-label={`Level ${level.id}: ${name}. ${status === "done" ? `Completed, ${stars} of 3 stars.` : "Locked."}`}
